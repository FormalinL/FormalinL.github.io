...
Player
```
那么其中我们可以看见，对于重新构造的函数，我们可以在其中加上override，这并不影响任何进程，但是可以有效地提示我们它是被重新构造的。<br>
[回到顶部](#top)

## Interfaces / Pure Virtual Funcitons ##
在C++中我们时常会用到被命名而未被定义的函数，即空函数。那么在类中，我们有时会在基类中命名一个方法（函数）而未定义它，转而选择在其各个子类中分别赋予其不同的解决方法，那么这种基类中的方法（函数）就是纯虚函数，又称接口。其定义方法就是在虚函数后加上=0
```
virtual void funtion()=0;
```
接口就是一个未包含实现方法的类，而也正因如此，我们无法将其实例化。若派生类中没有对纯虚函数进行定义，则在该派生类中其仍然为纯虚函数。
[回到顶部](#top)<br>

## Arrays ##
在C++中我们也经常使用数组来存储和使用一串数据。在C++中我们可以简单地将其分为栈数组和堆数组。堆数组的生存周期通常会长于栈数组，但同时其所需的操作时间一般也会长于栈数组，原因在于：栈数组是通过栈的内存加上偏移量来分配的，而堆数组是通过指针指向地址来保存数据的。可无论如何，我们都可以发现，在使用数组时都需要通过指针来进行保存，因此从某种意义上来说，数组就是指针。<br>
可以使用C++的Array来创建和使用数组，如：
```
include <array> //添加头文件
class Entity
{
...
>>>>>>> parent of 1c24605 (Update README.md)
